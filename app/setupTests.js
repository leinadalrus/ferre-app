...
class TestSuite {
  async openDebugApplicationTest () {
    ...
    testSignIn('Test User Sign-in functionality', async () => {
      try {
        let loginNavbarItem = await driver.findElement(
          By.xpath('/html/body/nav/div/div/div/ul/li[6]/a')
        )

        let emailInput = await driver.findElement(
          By.xpath(
            '/html/body/div[1]/div/section/div/div[2]/div/div[2]/form/div[1]/div/input'
          )
        )
        ...
        let signInButton = await driver.findElement(
          By.xpath(
            '/html/body/div[1]/div/section/div/div[2]/div/div[2]/form/div[3]/div/input'
          )
        )
    ...
    testPostJob('Test Post Job functionality.', async () => {
      try {
        let postAJobNavbarItem = await driver.findElement(
          By.xpath('/html/body/nav/div/div/div/ul/li[1]/a')
        )

        await postAJobNavbarItem.click()

        let postJobButton = await driver.findElement(By.xpath())
